...
export default function EditPackageScreen() {
  ...
  const { data: pkg, isLoading: loadingPackage } = usePackage(id || "");
  ...
  useEffect(() => {
    if (pkg) {
      setFormData({
        ...
        minPersons: pkg.minPersons?.toString() || "",
        maxPersons: pkg.maxPersons?.toString() || "",
  ...
  if (loadingPackage) {
    return <LoadingSpinner message="Loading package..." />;
  }
